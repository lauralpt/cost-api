...
public class ClearingCostController {
    ...
    public ResponseEntity<ClearingCost> getClearingCostByCountry(
        ...
        ClearingCost clearingCost = clearingCostService.findByCountryCode(countryCode);
        if (clearingCost == null) {
            throw new ResourceNotFoundException("Clearing cost for country code " + countryCode + " not found.");
        }
        return ResponseEntity.ok(clearingCost);
    }
    ...
    public ResponseEntity<ClearingCost> updateClearingCost(
            ...
            @Valid @RequestBody ClearingCost clearingCost) {
        ClearingCost updatedCost = clearingCostService.updateClearingCost(id, clearingCost);
        if (updatedCost == null) {
            throw new ResourceNotFoundException("Clearing cost with ID " + id + " not found.");
        }
    ...
    public ResponseEntity<BinlistDto.BinlistResponseWithCost> calculateClearingCost(
            @Parameter(description = "The payment card number (PAN). " +
        ...
        if (!clearingCostService.isPanValid(cardNumber)) {
            throw new IllegalArgumentException("Invalid card number. Please check the format and length.");
        }
        BinlistDto binlistDto = clearingCostService.getCountryCodeFromCardNumber(cardNumber);
        BigDecimal cost = clearingCostService.calculateClearingCost(cardNumber);
        return ResponseEntity.ok(
                new BinlistDto.BinlistResponseWithCost(binlistDto.getCountry().getAlpha2(), cost)
        );
    }
}